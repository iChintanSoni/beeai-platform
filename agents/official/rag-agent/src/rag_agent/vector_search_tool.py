...
class VectorSearchToolOutput(ToolOutput):
    ...
    @asynccontextmanager
    async def platform_client(self) -> AsyncIterator[httpx.AsyncClient]:
        """Create an async HTTP client for the platform API."""
        async with httpx.AsyncClient(base_url=f"{self._platform_url}/api/v1") as client:
            yield client
...
class VectorSearchTool(Tool[VectorSearchToolInput, ToolRunOptions, VectorSearchToolOutput]):
    ...
    def _create_emitter(self) -> Emitter:
        ...
        return Emitter.root().child(namespace=["tool", "vector_store_search"], creator=self)
